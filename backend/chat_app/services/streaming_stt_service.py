...
try:
    ...
    try:
        ...
        if isinstance(SILERO_UTILS, (list, tuple)) and len(SILERO_UTILS) >= 1:
            ...
            utils_tuple = SILERO_UTILS[0] if isinstance(SILERO_UTILS[0], (list, tuple)) else SILERO_UTILS
...
RNNOISE_USE_FFMPEG = True  # Prefer ffmpeg arnndn filter if available (no python bindings required)
...
def _denoise_via_ffmpeg(wav_bytes: bytes, target_sr: Optional[int] = None) -> bytes:
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            logger.debug(f"ffmpeg rnnoise failed: {proc.stderr.decode('utf-8', errors='ignore')}")
            return wav_bytes
...
def feed_audio(stream_id: str, wav_bytes: bytes) -> Optional[str]:
    ...
    if sess["last_voice_time"] and time_since_voice >= SILENCE_DURATION and total_duration >= MIN_UTTERANCE_DURATION:
        ...
        try:
            ...
            logger.info(f"Finalized utterance for stream {stream_id}, wrote {path} (duration={total_duration:.2f}s) "
    ...
    try:
        ...
        if sr != SILERO_SR:
            try:
                ...
                tgt_indices = np.linspace(0, tail.shape[0] - 1, num=int(round(tail_seconds * SILERO_SR)))
        ...
        try:
            ...
            speech_timestamps = get_ts(audio_tensor, SILERO_MODEL, sampling_rate=SILERO_SR)
        except Exception:
            # fallback to numpy
            try:
                speech_timestamps = get_ts(tail_rs, SILERO_MODEL, sampling_rate=SILERO_SR)
            except Exception:
                return None
