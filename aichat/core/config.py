...
from aichat.constants.paths import PROJECT_ROOT, MODELS_DIR
...
class Settings(BaseSettings):
    ...
    audio_input_device_id: Optional[int] = Field(
        default=None, env="AUDIO_INPUT_DEVICE_ID"
    )
    audio_output_device_id: Optional[int] = Field(
        default=None, env="AUDIO_OUTPUT_DEVICE_ID"
    )
    ...
    piper_model_path: Path = Field(
        default=MODELS_DIR / "en_US-amy-medium.onnx", env="PIPER_MODEL_PATH"
    )

    # Paths Configuration
    models_dir: Path = Field(default=MODELS_DIR, env="MODELS_DIR")
    ...
    character_personality: str = Field(
        default="cheerful,curious,helpful", env="CHARACTER_PERSONALITY"
    )
    ...
    openrouter_model: str = Field(
        default="qwen/qwen-2.5-7b-instruct", env="OPENROUTER_MODEL"
    )
    openrouter_url: str = Field(
        default="https://openrouter.ai/api/v1/chat/completions", env="OPENROUTER_URL"
    )
    ...
    def _setup_logging(self):
        ...
        logger.info(
            f"Logging initialized - Level: {self.log_level}, File: {self.log_file}"
        )
...
def validate_config() -> bool:
    ...
    try:
        ...
        if not settings.openrouter_api_key:
            logger.warning(
                "OpenRouter API key not configured - LLM functionality will be limited"
            )
