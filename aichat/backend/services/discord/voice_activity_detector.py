...
try:
    ...
    class MockVADConstants:
        def get_effective_energy_threshold(self):
            return -40.0

        def get_webrtc_aggressiveness(self):
            return 2
...
class DiscordVADAdapter:
    ...
    def __init__(self, discord_config):
        ...
        self.vad_service.set_callbacks(
            speech_end=self._on_vad_speech_end, vad_result=self._on_vad_result
        )
    ...
    async def process_audio_frame(
        self, user_id: int, audio_data: bytes, timestamp: float
    ) -> Optional[VADResult]:
        ...
        try:
            ...
            result = await self.vad_service.process_audio_frame(
                source_id, audio_data, timestamp
            )
            ...
            logger.error(
                f"Error processing audio frame for Discord user {user_id}: {e}"
            )
            return None

    async def _on_vad_speech_end(
        self, segment: SpeechSegment, audio_file: Optional[Path]
    ):
    ...
    def get_stats(self) -> Dict:
        ...
        discord_users = len(
            [
                s
                for s in vad_stats.get("sources", {}).keys()
