...
# Local imports
from aichat.constants.paths import GENERATED_AUDIO_DIR, TEMP_AUDIO_DIR, ensure_dirs
...
class AudioIOService:
    ...
    def _refresh_devices(self):
        ...
        try:
            ...
            logger.info(
                f"Refreshed devices: {len(self._input_devices)} input, {len(self._output_devices)} output"
            )
    ...
    async def record_audio(
        self, duration: float, output_path: Optional[Path] = None
    ) -> Optional[Path]:
        ...
        try:
            ...
            await self.event_system.emit(
                ...
                f"Started audio recording for {duration}s",
                {
                    ...
                    "device": (
                        self._current_input_device.name
                        if self._current_input_device
                        else "unknown"
                    ),
                },
            )
            ...
            await self.event_system.emit(
                ...
                f"Started audio playback: {audio_path.name}",
                {
                    ...
                    "device": (
                        ...
                        if self._current_output_device
                        else "unknown"
                    ),
                },
            )
    ...
    async def normalize_audio_format(
        ...
        try:
            ...
            if output_path is None:
                output_path = (
                    TEMP_AUDIO_DIR
                    / f"normalized_{int(time.time())}.{target_format.value}"
                )
    ...
    async def get_service_status(self) -> Dict[str, Any]:
        ...
        try:
            return {
                "status": "active",
                "backend": (
                    "pyaudio" if hasattr(self, "_pyaudio") and self._pyaudio else "mock"
                ),
                "config": {
                    "sample_rate": self.config.sample_rate,
                    "channels": self.config.channels,
