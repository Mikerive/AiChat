...
class ConfigTab(BaseComponent):
    ...
    def create(self):
        ...
        ttk.Label(api_frame, text="Host:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=2
        )
        ttk.Entry(api_frame, textvariable=self.api_host_var).grid(
            row=0, column=1, sticky=tk.EW, padx=5, pady=2
        )

        ttk.Label(api_frame, text="Port:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=2
        )
        ttk.Entry(api_frame, textvariable=self.api_port_var).grid(
        ...
        self.sample_rate_var = tk.StringVar(
            value=str(self.main_app.settings.sample_rate)
        )
        ...
        ttk.Entry(audio_frame, textvariable=self.sample_rate_var).grid(
            row=0, column=1, sticky=tk.EW, padx=5, pady=2
        )
        ...
        ttk.Entry(audio_frame, textvariable=self.channels_var).grid(
            row=1, column=1, sticky=tk.EW, padx=5, pady=2
        )
        ...
        self.whisper_model_var = tk.StringVar(
            value=self.main_app.settings.whisper_model
        )
        self.piper_model_var = tk.StringVar(
            value=str(self.main_app.settings.piper_model_path)
        )

        ttk.Label(model_frame, text="Whisper Model:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=2
        )
        ttk.Entry(model_frame, textvariable=self.whisper_model_var).grid(
            row=0, column=1, sticky=tk.EW, padx=5, pady=2
        )

        ttk.Label(model_frame, text="Piper Model:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=2
        )
        ttk.Entry(model_frame, textvariable=self.piper_model_var).grid(
            row=1, column=1, sticky=tk.EW, padx=5, pady=2
        )
        ...
        webhook_scroll = ttk.Scrollbar(
            webhooks_frame, orient=tk.VERTICAL, command=self.webhook_listbox.yview
        )
        ...
        remove_hook_btn = ttk.Button(
            webhook_buttons, text="Remove", command=self.remove_webhook
        )
        remove_hook_btn.pack(fill=tk.X, pady=(0, 5))
        test_hook_btn = ttk.Button(
            webhook_buttons, text="Test", command=self.test_webhook
        )
    ...
    def refresh_webhooks(self):
        ...
        try:
            response = requests.get(
                f"http://{self.main_app.settings.api_host}:{self.main_app.settings.api_port}/api/system/webhooks"
            )
    ...
    def test_webhook(self):
        ...
        try:
            ...
            msg = simpledialog.askstring(
                "Test Webhooks", "Test message (optional):", initialvalue="webhook test"
            )
            response = requests.post(
