...
class LogsTab(BaseComponent):
    ...
    def create(self):
        ...
        auto_refresh_check = ttk.Checkbutton(
            controls_frame, text="Auto Refresh", variable=self.auto_refresh_var
        )
        ...
        log_filter_combo = ttk.Combobox(
            controls_frame, textvariable=self.log_filter_var, width=15
        )
        ...
        clear_button = ttk.Button(
            self.frame, text="Clear Logs", command=self.clear_logs
        )
    ...
    def add_log(self, message: str, severity: str = "INFO"):
        ...
        try:
            ...
            if isinstance(message, dict) and (
                "error_code" in message or "http_code" in message
            ):
                log_data = message
            else:
                log_data = {
