...
from aichat.core.config import get_settings
            ...
            if self.main_app:
                self.main_app.add_log(
                    f"Audio stream sent: {stream_id} ({len(audio_data)} bytes)"
                )
    ...
    def send_chat_message(self, text: str, character: str = "hatsune_miku"):
        ...
        message = {"type": "chat", "text": text, "character": character}
        self.send_message(message)
    ...
    def _handle_audio_transcribed(self, data: Dict[str, Any]):
        ...
        if self.main_app:
            ...
            self.main_app.add_log(
                f"Transcription complete ({stream_id}): {text} (confidence: {confidence:.2f})"
            )
    ...
    def _handle_audio_generated(self, data: Dict[str, Any]):
        ...
        if self.main_app:
            ...
            self.main_app.add_log(
                f"TTS audio generated ({stream_id}): {character} - {audio_file}"
            )
    ...
    def handle_transcription_complete(self, message: str):
        ...
        try:
            ...
            if (
                data.get("type") == "transcription"
                and data.get("event") == "transcription_complete"
            ):
                stream_id = data.get("stream_id", "unknown")
                text = data.get("text", "")
    ...
    def handle_chat_complete(self, message: str):
        ...
        try:
            ...
            if (
                ...
                and data.get("event") == "response_ready"
            ):
                stream_id = data.get("stream_id", "unknown")
                data.get("user_input", "")
                ...
                if self.main_app:
                    self.main_app.add_log(
                        f"Chat Complete ({stream_id}): Response ready with audio"
                    )

                    # Add character response to chat
                    if hasattr(self.main_app, "chat_tab"):
                        self.main_app.chat_tab.add_chat_message(
        ...
        try:
            ...
            if message_type == "transcription":
                self._schedule_gui_update(
                    lambda: self.handle_transcription_complete(data)
                )
                return
                ...
                if self.main_app:
                    self._schedule_gui_update(
                        lambda: self.main_app.add_log(f"WebSocket Error: {error_msg}")
                    )
                return
                ...
                if self.main_app:
                    self._schedule_gui_update(
                        lambda: self.main_app.add_log("WebSocket: Connection alive")
                    )
                return
            ...
            if self.main_app:
                self._schedule_gui_update(
                    lambda: self.main_app.add_log(f"WS: {event_type} - {message_text}")
                )
            ...
            error_msg = f"WebSocket message error: {e}"
            logger.error(f"Error handling WebSocket message: {e}")
            if self.main_app:
                self._schedule_gui_update(
                    lambda: self.main_app.add_log(error_msg)
                )
