...
def update_job_json(job_path: Path, updates: dict):
    ...
    job.setdefault("history", []).append(
        {"ts": datetime.utcnow().isoformat() + "Z", "update": updates}
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train orchestrator for Piper fine-tuning"
    )
    parser.add_argument(
        "--dataset-dir",
        type=Path,
        default=DEFAULT_PIPER_DATASET,
        help="Preprocessed Piper dataset dir (output of preprocess step)",
    )
    parser.add_argument(
        "--raw-input-dir",
        type=Path,
        default=BASE_DIR / "training_data",
        help="Raw training data directory (used if preprocessing is requested)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="Where to store trained models and checkpoints",
    )
    parser.add_argument(
        "--model-name",
        type=str,
        default=f"piper_finetune_{int(time.time())}",
        help="Name for the output model folder",
    )
    parser.add_argument(
        "--language", type=str, default="en-us", help="Language for preprocessing"
    )
    parser.add_argument(
        "--dataset-format",
        choices=["ljspeech", "mycroft"],
        default="ljspeech",
        help="Dataset format for Piper preprocess",
    )
    parser.add_argument(
        "--sample-rate",
        type=int,
    ...
    parser.add_argument(
        "--single-speaker",
        action="store_true",
        help="Mark dataset as single-speaker for Piper preprocess",
    )
    parser.add_argument(
        "--accelerator",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Extra arguments passed to underlying commands",
    )
    parser.add_argument(
        "--log-file", type=Path, default=None, help="Training log file path"
    )
    ...
    if args.preprocess:
        ...
        try:
            preprocess_stdout = run_preprocess(
                args.raw_input_dir,
                preprocess_output,
                args.language,
                args.dataset_format,
            ...
            (model_output / "preprocess_stdout.txt").write_text(
                preprocess_stdout, encoding="utf-8"
            )
