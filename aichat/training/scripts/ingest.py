...
def extract_audio_to_wav(
    input_path: Path, output_wav: Path, sample_rate: int = DEFAULT_SAMPLE_RATE
) -> Path:
    """
...
def create_job_checkpoint(
    job_id: str, original_filename: str, checksum: str, duration_seconds: float = None
) -> Path:
    job = {
        "job_id": job_id,
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest audio/video file for TTS finetune pipeline"
    )
