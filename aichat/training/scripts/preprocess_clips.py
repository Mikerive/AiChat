...
from aichat.constants.paths import TTS_TRAINING_DIR, TTS_TRAINING_PROCESSED, ensure_dirs
...
def process_clip(in_path: Path, out_dir: Path, overwrite: bool = False) -> dict:
    try:
        ...
        if duration < MIN_AUDIO_LENGTH:
            logger.warning(
                f"Clip too short after processing ({duration:.2f}s): {in_path.name}"
            )
        ...
        with tempfile.NamedTemporaryFile(
            "w", delete=False, dir=str(out_dir), encoding="utf-8"
        ) as tmp:
...
def main():
    ...
    parser.add_argument(
        "--once", action="store_true", help="Process current queue and exit"
    )
    parser.add_argument(
