...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Piper manifest files from processed clips"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=DEFAULT_PROCESSED,
        help="Processed clips directory (wav files)",
    )
    parser.add_argument(
        "--metadata-csv",
        type=Path,
        default=DEFAULT_PROCESSED.parent / "metadata.csv",
        help="CSV with transcripts (filename,transcript,...)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT,
        help="Output directory for Piper dataset/manifest",
    )
    parser.add_argument(
        "--sample-rate",
        type=int,
        default=22050,
        help="Required sample rate for Piper dataset (22050 or 16000)",
    )
    parser.add_argument(
        "--format",
        choices=["ljspeech", "jsonl", "both"],
        default="both",
        help="Which manifest format(s) to generate",
    )
    parser.add_argument(
        "--resample",
        action="store_true",
        help="Resample/convert files that do not meet specs using ffmpeg",
    )
    parser.add_argument(
        "--copy-audio",
        action="store_true",
        help="Copy validated audio files into the output-dir/wavs/ (recommended)",
    )
