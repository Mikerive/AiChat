...
class StatusTab(BaseComponent):
    ...
    def create(self):
        ...
        ttk.Label(backend_frame, text="Backend Server:").pack(side=tk.LEFT, padx=(0, 10))
        ...
        self.start_backend_btn = ttk.Button(backend_frame, text="Start Backend", command=self.start_backend)
        self.start_backend_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.stop_backend_btn = ttk.Button(backend_frame, text="Stop Backend", command=self.stop_backend, state=tk.DISABLED)
        self.stop_backend_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.check_backend_btn = ttk.Button(backend_frame, text="Check Status", command=self.check_backend_status)
        ...
        self.ws_status_label = ttk.Label(ws_frame, text="Disconnected", foreground="red")
        self.ws_status_label.pack(side=tk.LEFT, padx=(0, 10))
        
        self.connect_ws_btn = ttk.Button(ws_frame, text="Connect WS", command=self.connect_websocket)
        self.connect_ws_btn.pack(side=tk.LEFT, padx=(0, 5))
        
        self.disconnect_ws_btn = ttk.Button(ws_frame, text="Disconnect WS", command=self.disconnect_websocket, state=tk.DISABLED)
        ...
        self.config_tree = ttk.Treeview(config_tree_frame, columns=columns, show="headings", height=6)
        ...
        self.metrics_tree = ttk.Treeview(metrics_tree_frame, columns=columns, show="headings")
        ...
        metrics_scrollbar = ttk.Scrollbar(metrics_tree_frame, orient=tk.VERTICAL, command=self.metrics_tree.yview)
        ...
        ttk.Button(button_frame, text="Refresh All", command=self.refresh_all_status).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Test Backend API", command=self.test_backend_api).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="View Logs", command=self.view_logs).pack(side=tk.LEFT)
    ...
    def check_backend_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.main_app.add_log(f"Backend responded with status: {response.status_code}")
    ...
    def update_config_status(self):
        ...
        # API Configuration
        self.config_tree.insert("", "end", values=("API Host", "OK", f"{settings.api_host}:{settings.api_port}"))
        
        # OpenRouter API Key
        if settings.openrouter_api_key:
            key_display = f"***{settings.openrouter_api_key[-4:]}" if len(settings.openrouter_api_key) > 4 else "***"
            self.config_tree.insert("", "end", values=("OpenRouter API Key", "✓ Configured", key_display))
        else:
            self.config_tree.insert("", "end", values=("OpenRouter API Key", "✗ Missing", "Not configured"))
        
        # Models
        self.config_tree.insert("", "end", values=("Whisper Model", "OK", settings.whisper_model))
        self.config_tree.insert("", "end", values=("Piper Model Path", "OK", str(settings.piper_model_path)))
        
        # Database
        self.config_tree.insert("", "end", values=("Database URL", "OK", settings.database_url))
        
        # Character
        self.config_tree.insert("", "end", values=("Character", "OK", f"{settings.character_name} ({settings.character_profile})"))
    ...
    def refresh_system_metrics(self):
        ...
        try:
            ...
            self.metrics_tree.insert("", "end", values=("Backend Status", "Unavailable - Cannot fetch metrics"))
    ...
    def update_system_metrics_display(self, status_data: Dict[str, Any]):
        ...
        # Add status items
        self.metrics_tree.insert("", "end", values=("Backend Status", status_data.get("status", "unknown")))
        self.metrics_tree.insert("", "end", values=("Uptime", f"{status_data.get('uptime', 0):.2f} seconds"))
        self.metrics_tree.insert("", "end", values=("CPU Usage", f"{status_data.get('cpu_usage', 0):.1f}%"))
        self.metrics_tree.insert("", "end", values=("Memory Usage", f"{status_data.get('memory_usage', 0):.1f}%"))
        
        disk_usage = status_data.get('disk_usage', {})
        if isinstance(disk_usage, dict):
            self.metrics_tree.insert("", "end", values=("Disk Usage", f"{disk_usage.get('percent', 0):.1f}%"))
        ...
        if 'requests_total' in status_data:
            self.metrics_tree.insert("", "end", values=("Total Requests", status_data['requests_total']))
        if 'active_connections' in status_data:
            self.metrics_tree.insert("", "end", values=("Active Connections", status_data['active_connections']))
