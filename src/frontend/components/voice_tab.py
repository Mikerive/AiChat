...
class VoiceTab(BaseComponent):
    ...
    def create(self):
        ...
        self.upload_button = ttk.Button(upload_frame, text="Upload Audio File", command=self.upload_audio)
        ...
        self.model_name_entry = ttk.Entry(controls_frame, textvariable=self.model_name_var)
        ...
        self.epochs_entry = ttk.Entry(controls_frame, textvariable=self.epochs_var, width=10)
        ...
        # Progress display
        self.progress_text = scrolledtext.ScrolledText(training_frame, wrap=tk.WORD, height=10)
        ...
        self.models_tree = ttk.Treeview(models_frame, columns=("Name", "Status", "Created"), show="headings", height=5)
        ...
        refresh_models_button = ttk.Button(models_frame, text="Refresh Models", command=self.refresh_models)
        ...
        if file_path:
            try:
                with open(file_path, "rb") as f:
                    files = {"file": f}
                    response = requests.post(f"{self.api_base_url}/api/voice/upload", files=files)
    ...
    def start_training(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.progress_text.insert(tk.END, f"Training started for {model_name}\n")
            else:
            ...
            if response.status_code == 200:
                ...
                for model in models:
                    self.models_tree.insert("", "end", values=(
                        model.get("name", ""),
                        model.get("status", ""),
                        model.get("created_at", "")[:19] if model.get("created_at") else ""
                    ))
        except Exception as e:
