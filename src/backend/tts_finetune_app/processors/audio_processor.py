...
def compute_sha256(path: Path) -> str:
    h = hashlib_sha256 = __import__("hashlib").sha256()
...
class AudioProcessor:
    ...
    def _frame_rms(self, audio: np.ndarray, frame_length: int = 2048, hop_length: int = 512) -> np.ndarray:
        return librosa.feature.rms(y=audio, frame_length=frame_length, hop_length=hop_length)[0]

    def split_on_silence_by_deviation(self,
                                      audio: np.ndarray,
                                      sr: int,
                                      max_segment_seconds: float,
                                      initial_factor: float = None,
                                      min_factor: float = None,
                                      factor_step: float = None) -> List[Tuple[float, float]]:
        """
        ...
        """
        initial_factor = initial_factor if initial_factor is not None else self.config.initial_factor
        min_factor = min_factor if min_factor is not None else self.config.min_factor
        factor_step = factor_step if factor_step is not None else self.config.factor_step
        ...
        std = float(np.std(rms)) if len(rms) else 1e-6
        ...
        def recursive_split(factor: float, segments_to_check: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
            new_segments = []
            for seg in segments_to_check:
                ...
                sub_rms = self._frame_rms(sub_audio, frame_length=frame_length, hop_length=hop_length)
                ...
                sub_times = librosa.frames_to_time(np.arange(len(sub_rms)), sr=sr, hop_length=hop_length, n_fft=frame_length)
                ...
                if not time_bounds:
                    ...
                    new_segments.extend(recursive_split(max(min_factor, factor - factor_step), [seg]))
                else:
                    # convert to global times
                    global_bounds = [(seg_start + s, seg_start + e) for s, e in time_bounds]
                    ...
                    for s_abs, e_abs in segments_created:
                        ...
                        if b_abs - a_abs > 0.01:
                            if b_abs - a_abs > max_segment_seconds:
                                new_segments.extend(recursive_split(max(min_factor, factor - factor_step), [(a_abs, b_abs)]))
                            else:
        ...
        result = [(max(0.0, a), min(audio_duration, b)) for a, b in merged if b - a > 0.05]
    ...
    def save_segments(self, audio: np.ndarray, sr: int, segments: List[Tuple[float, float]], base_name: str) -> List[Path]:
        saved = []
