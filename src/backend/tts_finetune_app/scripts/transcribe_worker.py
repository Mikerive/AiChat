...
def atomic_append_csv(csv_path: Path, row: list):
    ...
    with tempfile.NamedTemporaryFile("w", delete=False, dir=str(csv_path.parent), newline='', encoding='utf-8') as tmp:
        writer = csv.writer(tmp)
        writer.writerow(row)
        tmp_name = Path(tmp.name)
    # Append tmp file content to final CSV
    with csv_path.open("a", newline='', encoding='utf-8') as f_out, tmp_name.open("r", encoding='utf-8') as f_in:
...
class Transcriber:
    ...
    def transcribe_file(self, path: Path) -> dict:
        ...
        if self.whisper:
            ...
            raise RuntimeError("Whisper service not available in this environment. Please implement fallback or run inside app context.")
...
def process_once(transcriber: Transcriber, move_on_success: bool = True):
    ...
    if not METADATA_CSV.exists():
        with METADATA_CSV.open("w", newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["filename", "transcript", "duration_seconds", "speaker", "emotion", "quality"])
    ...
    for wav in sorted(AUDIO_DIR.glob("*.wav")):
        ...
        row = [wav.name, text.replace("\n", " ").strip(), duration or "", "target_speaker", "", "high"]
