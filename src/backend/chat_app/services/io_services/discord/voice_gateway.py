...
class VoiceGateway:
    ...
    async def connect(self) -> bool:
        ...
        if not VOICE_AVAILABLE:
            logger.error("Voice dependencies not available. Install with: pip install PyNaCl")
            return False
    ...
    async def _handle_hello(self, payload: Dict[str, Any]):
        ...
        logger.debug(f"Voice gateway HELLO received, heartbeat interval: {self._heartbeat_interval}s")
    ...
    async def _perform_ip_discovery(self):
        ...
        try:
            ...
            # Receive response
            data, addr = await asyncio.get_event_loop().sock_recvfrom(self.udp_socket, 74)
        ...
        try:
            ...
            if ssrc in self.sequence_numbers:
                ...
                if sequence != expected_seq:
                    logger.debug(f"Packet loss detected for SSRC {ssrc}: expected {expected_seq}, got {sequence}")
