...
from .config import DiscordConfig, AudioQuality
from .voice_gateway import VoiceGateway, VoicePacket
from .voice_activity_detector import DiscordVADAdapter, SpeechSegment
...
class VoiceReceiver:
    ...
    async def _handle_audio_packet(self, packet: VoicePacket):
        ...
        try:
            ...
            pcm_data = await self._decode_opus_audio(packet.user_id, packet.decrypted_data)
            if not pcm_data:
                return
            ...
            frame = AudioFrame(
                ...
                duration=len(pcm_data) / (self.target_sample_rate * self.target_channels * 2),  # 16-bit samples
    ...
    async def _decode_opus_audio(self, user_id: int, opus_data: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            if decoder:
                ...
                if self.target_sample_rate != 48000:
                    audio_array = self._resample_audio(audio_array, 48000, self.target_sample_rate)
    ...
    def _resample_audio(self, audio_array: np.ndarray, original_rate: int, target_rate: int) -> np.ndarray:
        ...
        try:
            ...
            return np.interp(indices, np.arange(len(audio_array)), audio_array).astype(np.int16)
    ...
    async def _on_speech_end(self, user_id: int, segment: SpeechSegment, audio_file: Optional[Path]):
        """Handle speech end event from VAD"""
        try:
            self.stats["speech_segments_detected"] += 1
            
            logger.debug(f"Speech ended for user {user_id} (duration: {segment.duration:.2f}s)")
    ...
    async def _save_speech_segment(self, segment: SpeechSegment) -> Optional[Path]:
        ...
        try:
            ...
            user_id = segment.source_id.replace("discord_user_", "") if segment.source_id.startswith("discord_user_") else segment.source_id
    ...
    async def export_user_audio(self, user_id: int, duration: float = 30.0) -> Optional[Path]:
