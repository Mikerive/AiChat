...
class AudioProcessor:
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info(f"Started {len(self.processing_tasks)} audio processing workers")
    ...
    async def stop(self):
        ...
        try:
            ...
            if self.active_transcriptions:
                await asyncio.gather(*self.active_transcriptions.values(), return_exceptions=True)
    ...
    async def process_speech_segment(self, user_id: int, segment: SpeechSegment, audio_file: Path):
        ...
        try:
            ...
            logger.debug(f"Queued speech segment for processing: user {user_id}, duration {segment.duration:.2f}s")
    ...
    async def _process_task(self, task: ProcessingTask, worker_name: str):
        ...
        try:
            logger.debug(f"Worker {worker_name} processing audio for user {task.user_id}")
    ...
    async def _transcribe_audio(self, task: ProcessingTask) -> Optional[TranscriptionResult]:
    ...
    async def _transcribe_with_whisper(self, audio_file: Path) -> Optional[Dict[str, Any]]:
    ...
    async def _handle_transcription_result(self, transcription: TranscriptionResult):
        ...
        try:
            ...
            logger.info(f"Transcription completed for {user.display_name}: '{transcription.text}'")
    ...
    async def _process_user_message(self, transcription: TranscriptionResult):
        ...
        try:
            ...
            message_data = {
                ...
                "confidence": transcription.confidence,
                "audio_file": str(transcription.audio_file) if transcription.audio_file else None
            }
            ...
            logger.debug(f"Processed user message from {user.display_name}: {transcription.text}")
