...
from .voice_activity_detector import SpeechSegment
...
class DiscordBot:
    ...
    async def start(self, bot_token: str) -> bool:
        ...
        if not DISCORD_AVAILABLE:
            logger.error("Discord.py not available. Install with: pip install discord.py[voice]")
            return False
    ...
    def _register_commands(self):
            ...
            try:
                if channel_id:
                    ...
                    if ctx.author.voice and ctx.author.voice.channel:
                        success = await self.join_voice_channel(ctx.author.voice.channel.id)
                    else:
                        await ctx.send("You need to be in a voice channel or specify a channel ID!")
                        return
        ...
        async def bot_status(ctx):
            ...
            try:
                ...
                embed = discord.Embed(title="VtuberMiku Discord Bot Status", color=0x00ff00)
                embed.add_field(name="Connected", value=status["connected"], inline=True)
                embed.add_field(name="Voice Channel", value=status.get("voice_channel", "None"), inline=True)
                embed.add_field(name="Users Tracked", value=status["users"]["total"], inline=True)
                embed.add_field(name="Speaking Users", value=status["users"]["speaking"], inline=True)
                embed.add_field(name="Uptime", value=f"{status['uptime']:.1f}s", inline=True)
        ...
        async def grant_consent(ctx):
            ...
            try:
                ...
                if success:
                    await ctx.send("Recording consent granted! Your voice will now be processed.")
                else:
        ...
        async def revoke_consent(ctx):
            ...
            try:
                ...
                if success:
                    await ctx.send("Recording consent revoked. Your voice will no longer be processed.")
                else:
    ...
    async def _on_speech_completed(self, user_id: int, segment: SpeechSegment, audio_file: Path):
        """Handle completed speech segment"""
        if self.audio_processor and audio_file:
            await self.audio_processor.process_speech_segment(user_id, segment, audio_file)
    ...
    async def set_services(self, whisper_service=None, tts_service=None, chat_service=None):
        ...
        try:
            ...
            return {
                ...
                "voice_channel": {
                    ...
                    "name": self.voice_client.channel.name if self.voice_client else None
                },
