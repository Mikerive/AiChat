...
class OpenRouterService:
    ...
    async def generate_response(
        ...
        try:
            if not self.api_key:
                ...
                return await self._generate_fallback_response(message, character_name, character_personality)
            
            # Build character context
            system_prompt = self._build_character_prompt(character_name, character_personality, character_profile)
    ...
    def _build_character_prompt(self, character_name: str, personality: str, profile: str) -> str:
...
- Respond naturally as if you're having a real conversation"""
    ...
    def _extract_emotion(self, response: str) -> str:
        ...
        # Emotion patterns
        if any(word in response_lower for word in ["happy", "excited", "joy", "great", "awesome", "wonderful"]):
            return "happy"
        elif any(word in response_lower for word in ["sad", "sorry", "disappointed", "upset"]):
            return "sad"
        elif any(word in response_lower for word in ["angry", "mad", "frustrated", "annoyed"]):
            return "angry"
        elif any(word in response_lower for word in ["surprised", "wow", "amazing", "incredible"]):
            return "surprised"
        elif any(word in response_lower for word in ["confused", "don't understand", "not sure", "unclear"]):
            return "confused"
        elif any(word in response_lower for word in ["calm", "peaceful", "relaxed", "serene"]):
            return "calm"
        else:
            return "neutral"
    ...
    async def _generate_fallback_response(self, message: str, character_name: str, personality: str) -> Dict[str, Any]:
