...
class AudioSourceTracker:
    ...
    def process_frame(self, audio_data: bytes, is_speech: bool, confidence: float, timestamp: float) -> Optional[SpeechSegment]:
    ...
    def _end_speech_segment(self, timestamp: float) -> Optional[SpeechSegment]:
        ...
        logger.debug(f"Ended speech segment for source {self.source_id} (duration: {completed_segment.duration:.2f}s)")
        return completed_segment
...
class VADService:
    ...
    def __init__(self, config: Optional[VADConfig] = None):
        ...
        logger.info(f"VAD Service initialized (WebRTC available: {WEBRTC_VAD_AVAILABLE})")
    ...
    async def process_audio_frame(self, source_id: str, audio_data: bytes, 
        ...
        try:
            ...
            is_speech, confidence = await self._detect_voice_activity(audio_data, audio_level_db)
            
            # Process through source tracker
            completed_segment = source.process_frame(audio_data, is_speech, confidence, timestamp)
            ...
            self.stats["active_sources"] = len([s for s in self.sources.values() 
                                              if s.current_state == VADState.SPEECH])
    ...
    def _remove_oldest_inactive_source(self):
        ...
        if inactive_sources:
            ...
            oldest_sid, oldest_source = min(inactive_sources, key=lambda x: x[1].last_activity)
    ...
    async def _detect_voice_activity(self, audio_data: bytes, audio_level_db: float) -> Tuple[bool, float]:
        """Detect voice activity using available methods"""
        is_speech = False
        confidence = 0.0
        
        # Method 1: WebRTC VAD (most reliable)
        if self.webrtc_vad and len(audio_data) == self.config.frame_size * 2:  # 16-bit samples
            try:
                webrtc_result = self.webrtc_vad.is_speech(audio_data, self.config.sample_rate)
        ...
        if not self.webrtc_vad:
            ...
            confidence = min(1.0, max(0.0, (audio_level_db + 60) / 20))  # Normalize -60dB to 0dB range
    ...
    async def _handle_speech_end(self, segment: SpeechSegment):
        ...
        try:
            ...
            logger.debug(f"Speech segment completed for source {segment.source_id} "
        ...
        try:
            while True:
                ...
                for source_id, source in self.sources.items():
                    if (current_time - source.last_activity > inactive_threshold and 
                        source.current_state == VADState.SILENCE):
    ...
    def get_active_sources(self) -> List[str]:
        ...
        return [
            source_id for source_id, source in self.sources.items()
