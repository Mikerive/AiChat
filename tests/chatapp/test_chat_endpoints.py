...
class MockChatService:
    ...
    async def process_message(self, text, character_id, character_name):
        return SimpleNamespace(
            response=f"{text}", emotion="neutral", model_used="mock-model"
        )
...
class MockDBOps:
    ...
    async def create_chat_log(
        self, character_id, user_message, character_response, emotion, metadata
    ):
    ...
    async def create_training_data(
        self, filename, transcript=None, duration=None, speaker=None, quality=None
    ):
...
def app_with_mocks(monkeypatch):
    ...
    monkeypatch.setattr(
        svc_mgr, "get_chat_service", lambda *args, **kwargs: mock_chat_service
    )
    ...
    app.dependency_overrides[voice_mod.get_voice_service_dep] = (
...
def test_switch_and_tts(app_with_mocks):
    ...
    r2 = client.post(
        "/api/chat/tts", json={"text": "speak", "character": "hatsune_miku"}
    )
    assert r2.status_code == 200
    t = r2.json()
...
def test_stt_upload(app_with_mocks, tmp_path):
    ...
    assert (
        "text" in body
        and isinstance(body["text"], str)
        and len(body["text"].strip()) > 0
    )
...
def test_whisper_model_initialized():
    ...
    model_info = asyncio.get_event_loop().run_until_complete(
        whisper_svc.get_model_info()
    )
...
def test_system_webhooks_and_status(app_with_mocks):
    ...
    r3 = client.delete(
        "/api/system/webhooks", params={"url": "https://example.com/hook"}
    )
