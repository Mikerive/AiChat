...
class DatabaseManager:
    ...
    async def get_session(self) -> AsyncGenerator[Any, None]:
        ...
        if aiosqlite is None:
            raise RuntimeError("aiosqlite is not installed; async database operations are unavailable in this environment")
...
# Database operations
async def create_character(name: str, profile: str, personality: str, avatar_url: Optional[str] = None) -> Character:
...
async def create_chat_log(character_id: int, user_message: str, character_response: str,
                        emotion: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> ChatLog:
    """Create a new chat log entry"""
    try:
        async with db_manager.get_session() as db:
            cursor = await db.execute(
                "INSERT INTO chat_logs (character_id, user_message, character_response, emotion, metadata) VALUES (?, ?, ?, ?, ?)",
                (character_id, user_message, character_response, emotion, str(metadata) if metadata else None)
            )
...
async def get_chat_logs(character_id: Optional[int] = None, limit: int = 100) -> List[ChatLog]:
    """Get chat logs"""
    try:
        logs = []
        async with db_manager.get_session() as db:
...
async def create_training_data(filename: str, transcript: Optional[str] = None,
                             duration: Optional[float] = None, speaker: str = "unknown",
                             emotion: Optional[str] = None, quality: Optional[str] = None) -> TrainingData:
...
async def create_voice_model(name: str, model_path: str, character_id: Optional[int] = None,
                          status: str = "created", epochs_trained: Optional[int] = None,
                          loss: Optional[float] = None) -> VoiceModel:
    ...
    try:
        async with db_manager.get_session() as db:
            ...
            # Get created model
            cursor = await db.execute(
                "SELECT * FROM voice_models WHERE id = ?",
                (model_id,)
            )
            row = await cursor.fetchone()
...
async def list_voice_models(limit: int = 100) -> List[VoiceModel]:
    ...
    try:
        ...
        async with db_manager.get_session() as db:
            cursor = await db.execute(
                "SELECT * FROM voice_models ORDER BY created_at DESC LIMIT ?",
                (limit,)
            )
...
async def log_event(event_type: str, message: str, data: Optional[Dict[str, Any]] = None,
                   severity: str = "INFO", source: Optional[str] = None) -> EventLog:
    """Log an event"""
    try:
        async with db_manager.get_session() as db:
            cursor = await db.execute(
...
if aiosqlite is None:
    ...
    async def _create_character(name: str, profile: str, personality: str, avatar_url: Optional[str] = None) -> Character:
        cid = _id_counters["characters"]
        _id_counters["characters"] += 1
        now = _dt.utcnow()
        ch = Character(id=cid, name=name, profile=profile, personality=personality, avatar_url=avatar_url, created_at=now, updated_at=now)
    ...
    async def _create_training_data(filename: str, transcript: Optional[str] = None,
                                    duration: Optional[float] = None, speaker: str = "unknown",
                                    emotion: Optional[str] = None, quality: Optional[str] = None) -> TrainingData:
        did = _id_counters["training_data"]
        _id_counters["training_data"] += 1
        now = _dt.utcnow()
        td = TrainingData(id=did, filename=filename, transcript=transcript, duration=duration, speaker=speaker, emotion=emotion, quality=quality, created_at=now)
    ...
    async def _get_chat_logs(character_id: Optional[int] = None, limit: int = 100) -> List[ChatLog]:
        logs = []
    ...
    async def _create_chat_log(character_id: int, user_message: str, character_response: str,
                            emotion: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> ChatLog:
        lid = _id_counters["chat_logs"]
        _id_counters["chat_logs"] += 1
        now = _dt.utcnow()
        cl = ChatLog(id=lid, character_id=character_id, user_message=user_message, character_response=character_response, emotion=emotion, metadata=metadata, timestamp=now)
